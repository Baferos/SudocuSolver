#include "SuducuSolver.h"


void SuducuSolver::solve(vector<vector<int>>& board) {
	solvePartialSudoku(0, 0, board);
	printBoard(board);
}

bool SuducuSolver::solvePartialSudoku(int row, int col, vector<vector<int>>& board) {
	int currentRow = row;
	int currentCol = col;
...
bool SuducuSolver::tryDigitsAtPosition(int row, int col, vector<vector<int>>& board) {
...
bool SuducuSolver::isValidAtPosition(int value, int row, int col,
...
void SuducuSolver::printBoard(vector<vector<int>>& board)
{
	cout << endl;
